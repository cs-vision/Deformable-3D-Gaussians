...
#     embed = lambda x, eo=embedder_obj: eo.embed(x) # AttributeError: Can't pickle local object 'get_embedder.<locals>.<lambda>'
#     return embed, embedder_obj.out_dim

# done this modification to avoid the pickling error from multiprocessing
# AttributeError: Can't pickle local object 'get_embedder.<locals>.<lambda>'
class EmbedderWrapper:
    def __init__(self, embedder_obj):
        self.embedder_obj = embedder_obj

    def __call__(self, x):
        return self.embedder_obj.embed(x)
...
def get_embedder(multires, i=1):
    ...
    embedder_obj = Embedder(**embed_kwargs)
    embed = EmbedderWrapper(embedder_obj)
    return embed, embedder_obj.out_dim
...
class Embedder:
    ...
    # got rid of lambda functions to avoid pickling error
    # AttributeError: Can't pickle local object 'Embedder.create_embedding_fn.<locals>.<lambda>'
    def create_embedding_fn(self):
        embed_fns = []
        d = self.kwargs['input_dims']
        out_dim = 0
        ...
        for freq in freq_bands:
            for p_fn in self.kwargs['periodic_fns']:
                self.freq = freq
                self.p_fn = p_fn
                embed_fns.append(self.periodic_fn)
                out_dim += d
